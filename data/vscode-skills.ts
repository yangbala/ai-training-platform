...
3. 批次自動化作業（批次重新命名、整理資料夾結構）`,
  ...
  },
  {
    ...
    context: '每個月都要整理銷售或出勤 CSV 資料？以前需要用 Excel 手動篩選，現在在 Codex 的對話框輸入一句話，30 秒內就能得到資料概況報告。Codex 會直接讀取你 VS Code 資料夾裡的檔案，不需要上傳或複製貼上內容。',
    examplePrompt: `（在 VS Code 開啟包含 sales.csv 的資料夾，切換到 Agent 模式，輸入：）

請讀取目前資料夾中的 sales.csv，告訴我：
1. 總共有幾筆資料
2. 哪些欄位有空值，各有幾筆
...
5. 2026-01-07｜林志豪｜產品B｜$8,200｜退款`,
    task: '準備一份你工作中實際用到的 CSV 檔案（或從網路下載範例 CSV），放到 VS Code 開啟的資料夾中，在 Agent 模式複製上方指令（把 sales.csv 換成你的檔名）執行看看。',
    checklist: [
      '我準備好一份 CSV 檔案並放到 VS Code 開啟的資料夾中',
      'Codex 成功讀取 CSV 並告訴我總筆數',
      'Codex 正確識別出有空值的欄位',
      '我嘗試追問「只列出金額超過 10,000 的資料」，Codex 也能回答',
    ],
  },
  {
    id: 4,
...
• 下季重點目標：提升舊客戶回購率至 30%。`,
    task: '找一份你工作中常用的 Word 文件或 PDF 報告，放到 VS Code 開啟的資料夾，讓 Codex 幫你摘要。試試看追加指示「只摘要關於財務的部分」。',
    checklist: [
